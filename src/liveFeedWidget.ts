...
(function(){
  ...
  /* ── Demo mode: show mock data without SSE ── */
  function runDemoData() {
    ...
    if (FEED_TYPE === 'agent-status') {
      handleData({ts: now, data: {
        summary: {running:3, queued:2, blocked:1},
        agents: [
          {id:'eng-1',  name:'Engineering Autopilot', domain:'engineering',  status:'running', currentTask:'Implement SSE fan-out logic',     workstream:'WS1: SSE Infrastructure'},
          {id:'des-1',  name:'Design Codex',           domain:'design',       status:'running', currentTask:'Radial progress rings',            workstream:'WS2: Widget Polish'},
          {id:'pro-1',  name:'Product Orchestrator',   domain:'product',      status:'running', currentTask:'Initiative pulse API',             workstream:'WS1: SSE Infrastructure'},
          {id:'mkt-1',  name:'Launch Captain',         domain:'marketing',    status:'queued',  currentTask:'Tweet thread draft',              workstream:'WS3: Go-to-Market'},
          {id:'ops-1',  name:'Control Tower',          domain:'operations',   status:'blocked', currentTask:'Waiting on deploy credentials',   workstream:'WS3: Go-to-Market'},
          {id:'xan-1',  name:'Xandy Orchestrator',     domain:'orchestration',status:'queued',  currentTask:'Coordinate launch sequence',      workstream:'WS3: Go-to-Market'}
        ]
      }});
      /* Tick updated timestamp every 10s for realism */
      setInterval(function(){
        updatedRow.textContent = 'Updated ' + fmt(Date.now());
      }, 10000);
    } else {
      handleData({ts: now, data: {
        initiatives: [{
          title: 'Operation Prism: Real-Time MCP Widgets',
          progress: 62,
          status: 'active',
          risk_level: 'medium',
          workstreamCount: 5,
          ...
          workstreams: [
            ...
            {name:'Go-to-Market',        domain:'marketing',    status:'queued',  progress:10},
            {name:'Credit Gate',         domain:'product',      status:'done',    progress:100},
            {name:'Deploy & Monitor',    domain:'operations',   status:'blocked', progress:20}
          ]
        }]
      }});
      /* Slowly increment progress every 12s */
      var _pct = 62;
      setInterval(function(){
        _pct = Math.min(100, _pct + 1);
        updatedRow.textContent = 'Updated ' + fmt(Date.now());
      }, 12000);
    }
  }

  var _demo = new URLSearchParams(location.search).get('demo') === 'true';
  if (_demo) { runDemoData(); } else { connect(); }
})();
</script>
</body>
</html>`;
