...
export const CONTRACT_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      'Use during a long-running session to read accepted material state changes since orgx_bootstrap. Pass the exact capsule_id and as_of_global_sequence returned by bootstrap. Returns only the currently ledger-backed allowlist in ascending global-sequence order: accepted/superseded decisions, authority lease changes, blocker changes, and metric-expectation registration/resolution. It explicitly reports material classes that are not ledger-backed yet. Read-only. USE WHEN: the session may have outlived its bootstrap context. NEXT: use next_after_sequence while has_more=true to inspect the feed, then bootstrap current context before consequential action. The capsule base is not verified; this feed cannot establish complete or coherent current state. DO NOT USE: as a substitute for orgx_bootstrap or to infer applied learning, constraints, or incidents that the coverage boundary marks unavailable.',
    inputSchema: {
      capsule_id: z
        .string()
