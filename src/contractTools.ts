...
export const CONTRACT_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      ...
      'Retry behavior: pass idempotency_key on creates; a persisted key match returns that UUID as an idempotent replay instead of creating a duplicate.\n\n' +
      'Initiative gotchas: priority only accepts low|medium|high|urgent, not portfolio labels such as active/critical/maintenance/hold; put portfolio urgency in metadata/priority_rank. due_date is not accepted on initiative create; put target dates in metadata.\n\n' +
  ...
  },
  {
    ...
    description:
      'Use when an entity\'s execution state must change so the next agent resumes from real status. Runs a lifecycle, validation, completion, or proof action on one OrgX entity. Per-action required inputs:\n' +
